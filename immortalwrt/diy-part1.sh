...
perl -0pi -e 's@ieee80211_hw_clear\(hw, SUPPORTS_AMSDU_IN_AMPDU\)@__clear_bit(IEEE80211_HW_SUPPORTS_AMSDU_IN_AMPDU, hw->flags)@g; s@\n\s*if \(changed & IEEE80211_RC_VHT_OMN_CHANGED\)\R\s*mt7996_mcu_set_fixed_field\(dev, link_sta, link, msta_link,\R\s*NULL, RATE_PARAM_VHT_OMN_UPDATE\);@\n\t\tif (changed & (IEEE80211_RC_SUPP_RATES_CHANGED |\n\t\t\t    IEEE80211_RC_NSS_CHANGED |\n\t\t\t    IEEE80211_RC_BW_CHANGED))@s; s@\n*else if[^\n]*IEEE80211_RC_SUPP_RATES_CHANGED[^\n]*\n[^\n]*IEEE80211_RC_NSS_CHANGED[^\n]*\n[^\n]*IEEE80211_RC_BW_CHANGED[^\n]*@@sg; s@\n#ifdef CONFIG_MTK_VENDOR\R\s*if \(changed & IEEE80211_RC_CODING_TYPE_CHANGED\) \{\R\s*struct sta_phy_uni phy = \{\R\s*\.ldpc = dev->coding_type,\R\s*\};\R\R\s*mt7996_mcu_set_fixed_field\(dev, link_sta, link, msta_link,\R\s*&phy, RATE_PARAM_FIXED_ENCODING\);\R\s*\}\R#endif@@s' "$D/mt7996/mac.c"
...
perl -0pi -e 's@wed->wlan\.wpdma_tx\[0\]@wed->wlan.wpdma_tx@g; s@wed->wlan\.hw_rro = \(enum mtk_wed_hwrro_mode\)dev->mt76\.hwrro_mode; /\* default on \*/@wed->wlan.hw_rro = mt7996_has_hwrro(dev); /* default on */@g' "$D/mt7996/mmio.c"
perl -0pi -e 's@\R\t\twed->wlan\.wpdma_tx\[1\] = wed->wlan\.phy_base \+ MT_TXQ_RING_BASE\(1\) \+\R\t\t\t\t\tMT7996_TXQ_BAND1 \* MT_RING_SIZE;@@; s@\R\t\telse\R\t\t\twed->wlan\.wpdma_rro_3_1_rx = wed->wlan\.phy_base \+\R\t\t\t\t\t     MT_RXQ_RRO_AP_RING_BASE \+\R\t\t\t\t\t     MT_RXQ_ID\(MT_RXQ_RRO_RXDMAD_C\) \* MT_RING_SIZE;@@; s@\R\t\t\} else \{\R\t\t\tif \(dev->mt76\.hwrro_mode == MT76_HWRRO_V3_1\)\R\t\t\t\twed->wlan\.rx_tbit\[0\] = ffs\(MT_INT_RX_DONE_WED_RX_DATA\) - 1;\R\t\t\telse\R\t\t\t\twed->wlan\.rx_tbit\[0\] = ffs\(MT_INT_RX_DONE_WED_RX_DATA_MT7996\) - 1;\R\t\t\}@@s' "$D/mt7996/mmio.c"
perl -0pi -e 's@\R[\t ]*wed->wlan\.wpdma_rro_3_1_rx = .*?;@@s' "$D/mt7996/mmio.c"
perl -0pi -e 's@\R\t\tif \(dev->mt76\.hwrro_mode == MT76_HWRRO_V3\)\R\t\t\twed->wlan\.wpdma_rx_pg = wed->wlan\.phy_base \+\R\t\t\t\t\tMT_RXQ_RING_BASE\(MT7996_RXQ_MSDU_PG_BAND0\) \+\R\t\t\t\t\tMT7996_RXQ_MSDU_PG_BAND0 \* MT_RING_SIZE;@\n\t\twed->wlan.wpdma_rx_pg = wed->wlan.phy_base +\n\t\t\t\t\tMT_RXQ_RING_BASE(MT7996_RXQ_MSDU_PG_BAND0) +\n\t\t\t\t\tMT7996_RXQ_MSDU_PG_BAND0 * MT_RING_SIZE;@s; s@\R\t\tif \(dev->mt76\.hwrro_mode == MT76_HWRRO_V3\) \{\R\t\t\twed->wlan\.rx_pg_tbit\[0\] = ffs\(MT_INT_RX_DONE_MSDU_PG_BAND0\) - 1;\R\t\t\twed->wlan\.rx_pg_tbit\[1\] = ffs\(MT_INT_RX_DONE_MSDU_PG_BAND1\) - 1;\R\t\t\twed->wlan\.rx_pg_tbit\[2\] = ffs\(MT_INT_RX_DONE_MSDU_PG_BAND2\) - 1;\R\t\t\} else \{\R\t\t\twed->wlan\.rro_3_1_rx_tbit = ffs\(MT_INT_RX_DONE_RRO_RXDMAD_C\) - 1;\R\t\t\}@\n\t\twed->wlan.rx_pg_tbit[0] = ffs(MT_INT_RX_DONE_MSDU_PG_BAND0) - 1;\n\t\twed->wlan.rx_pg_tbit[1] = ffs(MT_INT_RX_DONE_MSDU_PG_BAND1) - 1;\n\t\twed->wlan.rx_pg_tbit[2] = ffs(MT_INT_RX_DONE_MSDU_PG_BAND2) - 1;@s; s@\R\t\t\} else \{\R\t\t\}@@s; s@\R\t\telse\R\t\t\n@@s' "$D/mt7996/mmio.c"
