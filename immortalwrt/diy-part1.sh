...
perl -0pi -e 's@wed->wlan\.wpdma_tx\[0\]@wed->wlan.wpdma_tx@g; s@wed->wlan\.hw_rro = \(enum mtk_wed_hwrro_mode\)dev->mt76\.hwrro_mode; /\* default on \*/@wed->wlan.hw_rro = dev->has_rro; /* default on */@g' "$D/mt7996/mmio.c"
...
perl -0pi -e 's@\R\t\tif \(dev->mt76\.hwrro_mode == MT76_HWRRO_V3\)\R\t\t\twed->wlan\.wpdma_rx_pg = wed->wlan\.phy_base \+\R\t\t\t\t\tMT_RXQ_RING_BASE\(MT7996_RXQ_MSDU_PG_BAND0\) \+\R\t\t\t\t\tMT7996_RXQ_MSDU_PG_BAND0 \* MT_RING_SIZE;@\n\t\twed->wlan.wpdma_rx_pg = wed->wlan.phy_base +\n\t\t\t\t\tMT_RXQ_RING_BASE(MT7996_RXQ_MSDU_PG_BAND0) +\n\t\t\t\t\tMT7996_RXQ_MSDU_PG_BAND0 * MT_RING_SIZE;@s; s@\R\t\tif \(dev->mt76\.hwrro_mode == MT76_HWRRO_V3\) \{\R\t\t\twed->wlan\.rx_pg_tbit\[0\] = ffs\(MT_INT_RX_DONE_MSDU_PG_BAND0\) - 1;\R\t\t\twed->wlan\.rx_pg_tbit\[1\] = ffs\(MT_INT_RX_DONE_MSDU_PG_BAND1\) - 1;\R\t\t\twed->wlan\.rx_pg_tbit\[2\] = ffs\(MT_INT_RX_DONE_MSDU_PG_BAND2\) - 1;\R\t\t\}@\n\t\twed->wlan.rx_pg_tbit[0] = ffs(MT_INT_RX_DONE_MSDU_PG_BAND0) - 1;\n\t\twed->wlan.rx_pg_tbit[1] = ffs(MT_INT_RX_DONE_MSDU_PG_BAND1) - 1;\n\t\twed->wlan.rx_pg_tbit[2] = ffs(MT_INT_RX_DONE_MSDU_PG_BAND2) - 1;@s; s@\R\t\t\twed->wlan\.rro_3_1_rx_tbit = ffs\(MT_INT_RX_DONE_RRO_RXDMAD_C\) - 1;@@' "$D/mt7996/mmio.c"
